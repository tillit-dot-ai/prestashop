...
class Tillit {
    ...
    constructor()
    {
        ...
        const $billingCompany = $checkout.find('[name="company"]')

        // Get the company ID field
        const $companyId = $checkout.find('[name="companyid"]')
        ...
        if (tillitWithCompanySearch) {
            ...
            $billingCompany.select2({
                ...
                if (window.tillit.company_id_search) {
                ...
                const addressResponse = jQuery.ajax({
                    dataType: 'json',
                    url: window.tillit.tillit_checkout_host + '/v1/company/' + $checkout.find('[name="companyid"]').val() + '/address'
                });
                ...
                addressResponse.done(function (response) {
                    ...
                    if (response.company_location) {
                        ...
                        $checkout.find('[name="address1"]').val(companyLocation.street_address)

                        // Populate the city
                        $checkout.find('[name="city"]').val(companyLocation.municipality_name)

                        // Populate the postal code
                        $checkout.find('[name="postcode"]').val(companyLocation.postal_code)
    ...
    populateFields()
    {
        tillitCompany.company_name = $("input[name=company]").val()
        //tillitCompany.country_prefix = $("select[name=id_country]").val()
        tillitCompany.organization_number = $("input[name=companyid]").val()
        //tillitRepresentative.email = $("input[name=company]").val()
        tillitRepresentative.first_name = $("input[name=firstname]").val()
        tillitRepresentative.last_name = $("input[name=lastname]").val()
        tillitRepresentative.phone_number = $("input[name=phone]").val()
    }
    ...
    static getApproval()
    {
        ...
        approvalResponse.error(function (response) {
            ...
            if (response.status == 400) {
                ...
                if (jQuery)
                    jQuery.scroll_to_notices(jQuery('.woocommerce-NoticeGroup'));
            }
    ...
    changeAccountType()
    {
        ...
        if (accountType === 'business' && tillitApproved)
            tillitMethodHidden = false
...
jQuery(function () {
    if (window.tillit) {
        ...
        tillitWithCompanySearch = window.tillit.company_name_search
        new Tillit()
    }
})

$(document).ready(function () {
    typevalue = $('input:radio[name=account_type]:checked').val();
    if (typeof typevalue === "undefined") {
        typevalue = "business";
        $("input:radio[name=account_type][value=" + typevalue + "]").prop('checked', true);
    }
    showhidetype(typevalue);
    ...
    if (typeof prestashop !== 'undefined') {
        prestashop.on(
            ...
            function () {
                ...
                if (typeof typevalue === "undefined") {
                    typevalue = "business";
                    jQuery("input:radio[name=account_type][value=" + typevalue + "]").prop('checked', true);
                }
                showhidetype(typevalue);
        ...
        prestashop.on(
            'updateDeliveryForm',
            function () {
                typevalue = jQuery('input:radio[name=account_type]:checked').val();
                if (typeof typevalue === "undefined") {
                    typevalue = "business";
                    jQuery("input:radio[name=account_type][value=" + typevalue + "]").prop('checked', true);
                }
                ...
                jQuery("input[name='account_type']").click(function () {
                    value = $('input:radio[name=account_type]:checked').val();
                    showhidetype(value);
                });
                new Tillit()
            }
